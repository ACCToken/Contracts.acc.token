...
namespace eosio
{
	...
	public:
		...
		[[eosio::action]]
		void lock(const name& owner, const asset& quantity);

		[[eosio::action]]
		void release(const name& owner, const asset& quantity);

		[[eosio::action]]
		void dailyrelease(const name& owner, const asset& quantity);
		...
		static asset get_locked(const name& token_contract_account, const name& owner, const symbol_code& sym_code)
		{
			locks lockstable(token_contract_account, owner.value);
			const auto& ac = lockstable.get(sym_code.raw());
			return ac.locked;
		}
	...
	private:
		...
		struct [[eosio::table]] account_lock
		{
			asset locked;
			uint8_t release_status;
			asset release_per_day;
			uint64_t start_release_date;
			uint32_t released_days;

			uint64_t primary_key() const
			{
				return locked.symbol.code().raw();
			}
		};
